...
/// <summary>Validates the <see cref="CreatePurchaseOrderRequest"/> payload.</summary>
public sealed class CreatePurchaseOrderRequestValidator : AbstractValidator<CreatePurchaseOrderRequest>
{
    public CreatePurchaseOrderRequestValidator()
    {
        ...
        RuleFor(x => x.ExpectedDeliveryDate)
            .GreaterThan(x => x.DocumentDate).WithMessage("ExpectedDeliveryDate must be after DocumentDate.")
            .When(x => x.ExpectedDeliveryDate.HasValue && x.DocumentDate.HasValue);
        ...
        RuleFor(x => x.ContactEmail)
            .EmailAddress().WithMessage("ContactEmail is not a valid email address.");
