...
/// <summary>Validates the <see cref="CreateReminderRequest"/> payload.</summary>
public sealed class CreateReminderRequestValidator : AbstractValidator<CreateReminderRequest>
{
    public CreateReminderRequestValidator()
    {
        ...
        RuleFor(x => x.ReminderLevel)
            .InclusiveBetween(1, 10).WithMessage("ReminderLevel must be between 1 and 10.");
        ...
        RuleFor(x => x.ReminderFee)
            .GreaterThanOrEqualTo(0).WithMessage("ReminderFee must be greater than or equal to 0.")
            .When(x => x.ReminderFee.HasValue);
